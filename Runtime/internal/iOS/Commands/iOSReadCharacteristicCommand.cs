...
{
    public class iOSReadCharacteristicCommand : IDisposable
    {
        ...
        public iOSReadCharacteristicCommand(string deviceAddress, string serviceUuid, string characteristicUuid)
        {
        ...
        {
            if (_disposed)
                throw new ObjectDisposedException(nameof(iOSReadCharacteristicCommand));

            Debug.Log($"[iOS BLE Command] Reading characteristic {_characteristicUuid} from device {_deviceAddress}...");
            ...
            try
            {
                ...
                bool readStarted = iOSBleNativePlugin.ReadCharacteristic(_deviceAddress, _serviceUuid, _characteristicUuid);
                ...
                Debug.Log($"[iOS BLE Command] Read {result.Length} bytes from characteristic {_characteristicUuid}");
                
                return result;
            }
            catch (OperationCanceledException)
            {
                Debug.LogWarning($"[iOS BLE Command] Read operation timed out for characteristic {_characteristicUuid}");
                throw;
            }
            catch (Exception ex)
            {
                Debug.LogError($"[iOS BLE Command] Error reading characteristic {_characteristicUuid}: {ex.Message}");
                throw;
            }
            finally
            {
