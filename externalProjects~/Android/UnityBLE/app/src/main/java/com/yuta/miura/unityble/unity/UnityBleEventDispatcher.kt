...
import com.yuta.miura.unityble.dto.BleDeviceDTO
import com.yuta.miura.unityble.dto.BleServiceDTO
import com.yuta.miura.unityble.dto.SubscribeResponse
import com.yuta.miura.unityble.dto.ReadResponse
import com.yuta.miura.unityble.dto.WriteResponse
import com.yuta.miura.unityble.dto.toDto
...
class UnityBleEventDispatcher {
    companion object {
        ...
        private const val METHOD_NAME_ON_NOTIFY = "OnNotifyCharacteristic"
    }
    ...
    fun notifyResultForInvokeScan(result: ScanResult) {
        handler.post {
            UnityFacade.sendToUnity(GAME_OBJ_NAME, METHOD_NAME_SCAN_RESULT, result.name)
        }
    }

    fun notifyResultForInvokeStopScan(result: ScanResult) {
        handler.post {
            UnityFacade.sendToUnity(GAME_OBJ_NAME, METHOD_NAME_STOP_SCAN_RESULT, result.name)
        }
    }

    fun notifyResultForInvokeConnect(result: ConnectResult) {
        handler.post {
            UnityFacade.sendToUnity(GAME_OBJ_NAME, METHOD_NAME_CONNECT_RESULT, result.name)
        }
    }

    fun notifyResultForDiscoveryService(result: DiscoverServiceResult) {
        handler.post {
            UnityFacade.sendToUnity(GAME_OBJ_NAME, METHOD_NAME_DISCOVERY_RESULT, result.name)
        }
    }
    ...
    fun notifyOnSubscribe(from: BluetoothGattCharacteristic, value:String, result: SubscribeResult) {
        ...
        handler.post{
            UnityFacade.sendToUnity(GAME_OBJ_NAME, METHOD_NAME_ON_NOTIFY, str)
        }
    }
}